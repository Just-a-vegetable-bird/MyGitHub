...
public class HttpClient {
	...
	public Response post(String url, PostParameter[] postParams)
			throws Exception {
	...
	public Response post(String url, PostParameter[] postParams, boolean login)
			throws Exception {
		// return httpRequest(url,postParams,false,false);
	 ...
	 * 
	 * 
	 * if (authenticated) { if (basic == null && oauth == null) { } String
	 * authorization = null; if (null != oauth) { // use OAuth authorization =
	 * oauth.generateAuthorizationHeader( params != null ? "POST" : "GET", url,
	 * params, oauthToken); } else if (null != basic) { // use Basic Auth
	 * authorization = this.basic; } else { throw new IllegalStateException(
	 * "Neither user ID/password combination nor OAuth consumer key/secret combination supplied"
	 * ); } connection.addRequestProperty("Authorization", authorization);
	 * log("Authorization: " + authorization); } for (String key :
	 * requestHeaders.keySet()) { connection.addRequestProperty(key,
	 * requestHeaders.get(key)); log(key + ": " + requestHeaders.get(key)); } }
	 */
